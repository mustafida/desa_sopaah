...
async function run() {
	...
	await pool.query('INSERT IGNORE INTO pengaturan (kunci, nilai) VALUES (?, ?)', [
		'visi_misi',
		JSON.stringify(visiMisi)
	]);
	await pool.query('INSERT IGNORE INTO pengaturan (kunci, nilai) VALUES (?, ?)', [
		'data_kependudukan',
		JSON.stringify(dataKependudukan)
	]);
	await pool.query('INSERT IGNORE INTO pengaturan (kunci, nilai) VALUES (?, ?)', [
		'data_pendidikan',
		JSON.stringify(dataPendidikan)
	]);
	await pool.query('INSERT IGNORE INTO pengaturan (kunci, nilai) VALUES (?, ?)', [
		'data_pekerjaan',
		JSON.stringify(dataPekerjaan)
	]);
