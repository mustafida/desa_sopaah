	...
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
		<div>
			...
			<p class="text-slate-500 text-sm mt-1">
				Tambah, edit, atau hapus berita dan pengumuman desa.
			</p>
		</div>
		<button
			onclick={() => {
		...
		>
			...
			>
				...
				>
					...
					<div>
						<label for="isi" class="block text-sm font-medium text-slate-700 mb-1.5"
							>Isi Berita *</label
						>
						<textarea
							id="isi"
							...
							placeholder="Tulis isi berita...">{editingItem?.isi ?? ''}</textarea
						>
					</div>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
						<div>
							...
							<p class="text-xs text-slate-400 mt-1">
								Pilih kategori <strong>Pengumuman</strong> agar tulisan masuk ke Papan Pengumuman di halaman
								depan.
							</p>
						</div>
						<div>
					...
					<div>
						<label for="created_at" class="block text-sm font-medium text-slate-700 mb-1.5"
						...
						<input
							...
							id="created_at"
							name="created_at"
							value={toLocalDateString(editingItem?.createdAt)}
							required
							class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all text-slate-800"
						/>
						<p class="text-xs text-slate-400 mt-1">
							Tanggal publikasi berita. Berguna jika ingin mempublikasikan dengan tanggal lampau
							(backdate) atau tanggal khusus.
						</p>
					</div>
					...
					<div class="flex items-center gap-3">
						...
						<label for="is_featured" class="text-sm font-medium text-slate-700"
							>Jadikan Berita Utama (Featured)</label
						>
					</div>
				...
				<table class="w-full text-left">
					<thead class="bg-slate-50 border-b border-slate-200">
						<tr>
							...
							>
							<th
								class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell"
								>Kategori</th
							>
							<th
								class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden sm:table-cell"
								>Tanggal</th
							>
							<th
						...
						{#each data.beritaList as item}
							<tr class="hover:bg-slate-50 transition-colors">
								...
								<td class="px-6 py-4 hidden md:table-cell">
									<span
										class="text-xs px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-medium"
										>{item.kategori}</span
									>
								</td>
									...
									<div class="flex items-center justify-end gap-2">
										...
										{#if deleteConfirmId === item.id}
											...
											>
												...
												<button
													type="submit"
													class="px-2 py-1 text-xs bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"
													>Yakin?</button
												>
												<button
													type="button"
													onclick={() => (deleteConfirmId = null)}
													class="px-2 py-1 text-xs bg-slate-200 text-slate-600 rounded-lg hover:bg-slate-300 transition-colors"
													>Batal</button
												>
											</form>
										{:else}
