<script lang="ts">
	...
	$effect(() => {
		ketua = data.struktur?.ketua ?? '';
		wakil = data.struktur?.wakil ?? '';
		sekretaris = data.struktur?.sekretaris ?? '';
		bendahara = data.struktur?.bendahara ?? '';
		deskripsi = data.struktur?.deskripsi ?? '';
		gambarUrl = data.struktur?.gambarUrl ?? '';
		pokja1 = data.struktur?.pokja1 ?? '';
		pokja2 = data.struktur?.pokja2 ?? '';
		pokja3 = data.struktur?.pokja3 ?? '';
		pokja4 = data.struktur?.pokja4 ?? '';
	});
</script>
...
<div class="max-w-3xl mx-auto">
	<div class="mb-6">
		...
		<p class="text-slate-500 text-sm mt-1">
			Kelola nama-nama pengurus inti Tim Penggerak PKK Desa Sopaah.
		</p>
	</div>
			...
			>
				...
				<div>
					<label for="deskripsi" class="block text-sm font-semibold text-slate-700 mb-2"
						>Deskripsi (Tujuan Gerakan PKK)</label
					>
					<textarea
						id="deskripsi"
						name="deskripsi"
						...
						placeholder="Tuliskan tujuan gerakan PKK..."></textarea>
				</div>

				<div>
					<label for="gambar" class="block text-sm font-semibold text-slate-700 mb-2"
						>Foto/Gambar (Banner PKK)</label
					>
					{#if gambarUrl}
						<div class="mb-3 relative w-fit">
							<img
								src={gambarUrl}
								alt="Foto PKK"
								class="h-32 object-contain rounded-xl border border-slate-200"
							/>
				...
				<div>
					<label for="ketua" class="block text-sm font-semibold text-slate-700 mb-2"
						>Ketua TP PKK</label
					>
					<input
						type="text"
				...
				<div>
					<label for="wakil" class="block text-sm font-semibold text-slate-700 mb-2"
						>Wakil Ketua</label
					>
					<input
						type="text"
				...
				<h2 class="text-lg font-bold text-slate-800 mb-4 border-b pb-2 mt-8">
					Pembagian Pokja (Kelompok Kerja)
				</h2>
				<p class="text-sm text-slate-500 mb-4">
					Isi dengan nama Ketua Pokja atau deskripsi singkat tugas masing-masing Pokja.
				</p>

				<div class="grid sm:grid-cols-2 gap-6">
					<div>
						<label for="pokja1" class="block text-sm font-semibold text-slate-700 mb-2"
							>Pokja I</label
						>
						<textarea
							id="pokja1"
							...
							placeholder="Membidangi Penghayatan & Pengamalan Pancasila serta Gotong Royong."
						></textarea>
					</div>
					<div>
						<label for="pokja2" class="block text-sm font-semibold text-slate-700 mb-2"
							>Pokja II</label
						>
						<textarea
							id="pokja2"
							...
							rows="2"
							class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-pink-500 bg-slate-50"
							placeholder="Membidangi Pendidikan, Keterampilan, dan Pengembangan Berkoperasi."
						></textarea>
					</div>
					<div>
						<label for="pokja3" class="block text-sm font-semibold text-slate-700 mb-2"
							>Pokja III</label
						>
						<textarea
							id="pokja3"
							...
							rows="2"
							class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-pink-500 bg-slate-50"
							placeholder="Membidangi Pangan, Sandang, Perumahan & Tata Laksana Rumah Tangga."
						></textarea>
					</div>
					<div>
						<label for="pokja4" class="block text-sm font-semibold text-slate-700 mb-2"
							>Pokja IV</label
						>
						<textarea
							id="pokja4"
							...
							rows="2"
							class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-pink-500 bg-slate-50"
							placeholder="Membidangi Kesehatan, Kelestarian Lingkungan Hidup, dan Perencanaan Sehat."
						></textarea>
					</div>
				</div>
				...
				<div class="mt-8 pt-6 border-t border-slate-100 flex justify-end">
					<button
						type="submit"
						class="px-6 py-2.5 bg-pink-600 text-white font-medium rounded-xl hover:bg-pink-700 transition-colors shadow-sm flex items-center gap-2"
					>
			...
			{:else}
				...
				>
					...
					<p class="text-slate-500 max-w-sm mx-auto mt-2">
						Tambahkan foto-foto kegiatan PKK untuk ditampilkan di halaman pengunjung.
					</p>
				</div>
			{/each}
		</div>
