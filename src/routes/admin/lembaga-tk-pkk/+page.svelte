<script lang="ts">
	...
	$effect(() => {
		ketuaYayasan = data.struktur?.ketuaYayasan ?? '';
		kepalaSekolah = data.struktur?.kepalaSekolah ?? '';
		operatorSekolah = data.struktur?.operatorSekolah ?? '';
		sekretaris = data.struktur?.sekretaris ?? '';
		bendahara = data.struktur?.bendahara ?? '';
		guruKelompokA = data.struktur?.guruKelompokA ?? '';
		guruKelompokB = data.struktur?.guruKelompokB ?? '';
		npsn = data.struktur?.npsn ?? '';
		status = data.struktur?.status ?? '';
		akreditasi = data.struktur?.akreditasi ?? '';
		alamat = data.struktur?.alamat ?? '';
		email = data.struktur?.email ?? '';
		noWhatsapp = data.struktur?.noWhatsapp ?? '';
		visi = data.struktur?.visi ?? '';
		misiArray = data.struktur?.misi ?? [];
		tujuanArray = data.struktur?.tujuan ?? [];
	});
</script>
...
<div class="max-w-4xl mx-auto">
	<div class="mb-6">
		...
		<p class="text-slate-500 text-sm mt-1">
			Kelola data profil sekolah, struktur kepengurusan, visi & misi, serta dokumentasi galeri.
		</p>
	</div>
	...
	<div class="flex gap-6 border-b border-slate-200 mb-6">
		...
		<button
			...
			class="pb-3 text-sm font-semibold transition-colors relative {activeTab === 'profil'
				? 'text-yellow-600'
				: 'text-slate-500 hover:text-slate-700'}"
		>
			Profil & Kontak
		...
		<button
			...
			class="pb-3 text-sm font-semibold transition-colors relative {activeTab === 'dokumentasi'
				? 'text-yellow-600'
				: 'text-slate-500 hover:text-slate-700'}"
		>
	...
	<!-- Form Settings -->
	<div
		class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden {activeTab ===
		'dokumentasi'
			? 'hidden'
			: 'block'}"
	>
		<form method="POST" action="?/simpan" use:enhance class="p-6 sm:p-8 space-y-8">
			...
			<div class={activeTab === 'struktur' ? 'block space-y-6' : 'hidden'}>
				...
				<div class="grid md:grid-cols-2 gap-6">
					<div>
						<label for="ketuaYayasan" class="block text-sm font-semibold text-slate-700 mb-2"
							>Ketua Yayasan</label
						>
						<input
							type="text"
					...
					<div>
						<label for="kepalaSekolah" class="block text-sm font-semibold text-slate-700 mb-2"
					...
					<div>
						<label for="operatorSekolah" class="block text-sm font-semibold text-slate-700 mb-2"
							>Operator Sekolah</label
						>
						<input
							type="text"
							id="operatorSekolah"
					...
					<div>
						<label for="sekretaris" class="block text-sm font-semibold text-slate-700 mb-2"
							>Sekretaris</label
						>
						<input
							type="text"
					...
					<div>
						<label for="bendahara" class="block text-sm font-semibold text-slate-700 mb-2"
							>Bendahara</label
						>
						<input
							type="text"
					...
					<div>
						<label for="guruKelompokA" class="block text-sm font-semibold text-slate-700 mb-2"
							>Guru Kelompok A</label
						>
						<input
							type="text"
							id="guruKelompokA"
					...
					<div class="md:col-span-2">
						<label for="guruKelompokB" class="block text-sm font-semibold text-slate-700 mb-2"
							>Guru Kelompok B (Gunakan baris baru / Enter untuk nama lebih dari satu)</label
						>
						<textarea
							id="guruKelompokB"
							name="guruKelompokB"
			...
			<div class={activeTab === 'profil' ? 'block space-y-6' : 'hidden'}>
				...
				<div class="grid md:grid-cols-2 gap-6">
					<div>
						<label for="npsn" class="block text-sm font-semibold text-slate-700 mb-2"
							>NPSN Sekolah</label
						>
						<input
							type="text"
					...
					<div>
						<label for="akreditasi" class="block text-sm font-semibold text-slate-700 mb-2"
							>Status Akreditasi</label
						>
						<input
							type="text"
							id="akreditasi"
					...
					<div>
						<label for="noWhatsapp" class="block text-sm font-semibold text-slate-700 mb-2"
							>Nomor WhatsApp Kontak (Tanpa angka 0 di depan)</label
						>
						<div class="relative">
			...
			<div class={activeTab === 'visimisi' ? 'block space-y-6' : 'hidden'}>
				...
				<div class="mb-6">
					<label for="visi" class="block text-sm font-semibold text-slate-700 mb-2"
						>Visi Sekolah</label
					>
					<textarea
						id="visi"
				...
				<div class="border-t border-slate-100 pt-6">
					<div class="flex items-center justify-between mb-4">
						...
						<button
							type="button"
							onclick={addMisi}
							class="text-xs px-3 py-1.5 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 rounded-lg font-bold transition-colors"
						>
							+ Tambah Misi
						</button>
					</div>
					...
					<div class="space-y-3">
						...
						{#if misiArray.length === 0}
							<div
								class="text-center p-6 border border-dashed border-slate-200 rounded-xl text-slate-400 text-sm"
							>
	...
	<div class={activeTab === 'dokumentasi' ? 'block' : 'hidden'}>
		...
		{#if isAddingDoc}
			<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
				...
				>
					<div class="grid md:grid-cols-2 gap-6">
						<div>
							<label for="judul_doc" class="block text-sm font-semibold text-slate-700 mb-2"
								>Judul/Keterangan Foto</label
							>
							<input
								type="text"
								id="judul_doc"
						...
						<div>
							<label for="gambar_doc" class="block text-sm font-semibold text-slate-700 mb-2"
								>Pilih Foto</label
							>
							<input
								type="file"
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
				...
				<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group">
					...
					<div class="p-5 flex justify-between items-start gap-4">
						...
						<form
							...
							use:enhance
							onsubmit={() => confirm('Yakin ingin menghapus foto dokumentasi ini?')}
						>
							<input type="hidden" name="id" value={foto.id} />
							<button
								type="submit"
								class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors shrink-0"
								title="Hapus Foto"
							>
			...
			{:else}
				<div
					class="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50"
				>
					<svg
					...
					<p class="text-slate-500 max-w-sm mx-auto mt-2">
						Tambahkan foto-foto kegiatan TK PKK untuk ditampilkan di halaman pengunjung.
					</p>
				</div>
			{/each}
		</div>
